...
class GainValidator:
    ...
    def __init__(self, gains_file: str = "optimized_gains.json"):
        ...
        try:
            ...
            logging.error(
                f"Gains file not found: {gains_file}. Please run reoptimize_controllers.py first."
            )
            sys.exit(1)
    ...
    def validate_model_consistency(self, controller_type: str) -> Dict:
        ...
        for test_name, state, control in test_conditions:
            ...
            metrics = compare_models(
                simple_dynamics, full_dynamics, state, control, dt=0.01
            )
    ...
    def generate_validation_report(self):
        ...
        for controller_type in self.gains.keys():
            ...
            max_deriv_error = (
                max([r["derivative_error"] for r in model_results.values()])
                if model_results
                else 0
            )
            report.append(
                f"{'PASS' if max_deriv_error < 0.1 else 'WARNING'} (max deriv error: {max_deriv_error:.4f})\n"
            )
