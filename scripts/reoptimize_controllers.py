...
class ControllerReoptimizer:
    ...
    def optimize_controller(self, controller_type: str, n_runs: int = 3) -> Dict:
        ...
        for run in range(n_runs):
            ...
            tuner = PSOTuner(controller_factory, config_path=str(self.results_dir / "config.yaml"))
            ...
            logging.info(f"Best cost: {result['best_cost']:.6f} | Best gains: {result['best_pos']}")
    ...
    def _create_comparison_report(self, all_results: Dict):
        ...
        report = [f"# Controller Optimization Report\n\nDate: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"]
...
def main(argv: Optional[list[str]] = None):
    ...
    parser = argparse.ArgumentParser(description="Systematic re-optimization of all controller types.")
    parser.add_argument(
        "--config",
        default="config.yaml",
    ...
    parser.add_argument("--save-json", type=str, help="Save final gains to a JSON file.")
    parser.add_argument(
        "--hyper-tune",
        action="store_true",
    ...
    if args.hyper_tune:
        ...
        for ctrl in controller_types:
            ...
            from src.optimizer.pso_optimizer import PSOTuner
            tuner = PSOTuner(controller_factory, config_path=str(optimizer.results_dir / "config.yaml"))
            ...
            logging.info(f"[{ctrl}] Best cost found during study: {hyper['best_value']:.6f}")
    ...
    if args.save_json:
        optimized_gains = {ctrl: data['best_gains'] for ctrl, data in final_results.items()}
        with open(args.save_json, 'w') as f:
