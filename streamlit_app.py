...
def _parse_initial_state(text: str, default: np.ndarray, state_dim: int) -> np.ndarray:
    ...
    if arr.size < state_dim:
        ...
        st.warning(
            f"Input has {arr.size} values, but {state_dim} are required. Truncating excess values."
        )
        arr = arr[:state_dim]
    return arr
...
def _compute_metrics(
    ...
    metrics = {
        ...
        "peak_abs_th1_deg": (
            float(np.rad2deg(np.max(np.abs(x[:, 1])))) if x.shape[1] > 1 else 0.0
        ),
        "peak_abs_th2_deg": (
            float(np.rad2deg(np.max(np.abs(x[:, 2])))) if x.shape[1] > 2 else 0.0
        ),
...
def main():
    ...
    best_params = np.array(
        cfg.controller_defaults[controller_sel]["gains"], dtype=float
    )
    if st.sidebar.button(t.get("run_button", "Run PSO")):
        with st.spinner(t.get("spinner_msg", "Optimizing...")):
            controller_factory = lambda gains: create_controller(
                controller_sel, config=cfg, gains=gains
            )
            tuner = PSOTuner(
                controller_factory, config=cfg, seed=getattr(cfg, "global_seed", None)
            )
            ...
            if np.isfinite(res.get("best_cost", np.inf)):
                ...
                st.metric(
                    label=t.get("cost_metric", "Best Cost"),
                    value=f"{res['best_cost']:.4f}",
                )
            else:
                st.error(
                    t.get("opt_fail", "Optimization failed to find a valid solution.")
                )
    ...
    add_disturbance = st.sidebar.checkbox(
        t.get("add_disturbance_label", "Add Disturbance"), value=False
    )
    ...
    st.write(
        t.get(
            "intro",
            "Real‑time PSO‑tuned controllers for double‑inverted pendulum control.",
        )
    )
    ...
    with st.expander(t.get("time_series_header", "Time‑Series (x, θ1, θ2, u)")):
        ...
        if disturbance_function is not None:
            ...
            axes[3].plot(
                t_sim[:-1], d_values, "orange", linestyle="--", label="Disturbance"
            )
            axes[3].legend()
    ...
    with cols[0]:
        st.metric(
            t.get("settling_time_label", "Settling Time"),
            f"{metrics['settling_time_s']:.2f} s",
        )
        st.metric(
            t.get("rms_control_label", "RMS Control"),
            f"{metrics['rms_control_N']:.2f} N",
        )
    with cols[1]:
        st.metric(t.get("peak_x_label", "Peak |x|"), f"{metrics['peak_abs_x_m']:.3f} m")
        st.metric(
            t.get("peak_theta1_label", "Peak |θ1|"),
            f"{metrics['peak_abs_th1_deg']:.1f}°",
        )
    with cols[2]:
        st.metric(
            t.get("peak_theta2_label", "Peak |θ2|"),
            f"{metrics['peak_abs_th2_deg']:.1f}°",
        )
    ...
    with zipfile.ZipFile(zip_buf, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        writer.writerow(
            ["time", "x", "theta1", "theta2", "x_dot", "theta1_dot", "theta2_dot", "u"]
        )
    ...
    if st.session_state.lang == "فارسی":
        st.markdown(
            "<style>div[data-testid='stSidebar'] > div {direction: rtl;}</style>",
