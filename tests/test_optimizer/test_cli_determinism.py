...
def _create_fast_config(src_config: Path, dst_config: Path):
    ...
    cfg = {
        "global_seed": 42,
        "controller_defaults": {
            "classical_smc": {"gains": [5.0, 5.0, 5.0, 0.5, 0.5, 0.5]}
        },
        "controllers": {
            "classical_smc": {
                "max_force": 150.0,
        ...
        "pso": {
            ...
            "seed": 42,
            "hyper_trials": 50,
            "hyper_search": {"w": [0.1, 0.9], "c1": [0.5, 2.5], "c2": [0.5, 2.5]},
            "study_timeout": 600,
            "tune": {"classical_smc": {"w": 0.5, "c1": 1.5, "c2": 1.5}},
        },
        "physics": {
            "cart_mass": 1.5,
            "pendulum1_mass": 0.2,
        ...
        "verification": {
            "test_conditions": [{"name": "nominal", "disturbance": [0, 0, 0]}],
            "integrators": ["RK45"],
            "criteria": {"position_tolerance": 0.01, "angle_tolerance": 0.01},
        },
        "cost_function": {
            "weights": {
                "state_error": 50.0,
            ...
            "baseline": {"classical_smc": 100.0, "sta_smc": 90.0, "adaptive_smc": 85.0},
        },
        "sensors": {
            "angle_noise_std": 0.0,
...
def test_cli_stdout_is_deterministic(tmp_path: Path):
    ...
    if c1 is None or g1 is None or c2 is None or g2 is None:
        pytest.skip(
            "Could not find 'Best Cost'/'Best Gains' lines in output; adjust regex if needed."
        )
