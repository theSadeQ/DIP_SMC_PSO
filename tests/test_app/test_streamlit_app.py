...
def make_fake_streamlit():
    ...
    sidebar = types.SimpleNamespace(
        ...
        selectbox=lambda *args, **kwargs: (
            kwargs.get("options") or args[1] if len(args) > 1 else ["SMC"]
        )[0],
        button=lambda *args, **kwargs: False,
        write=lambda *args, **kwargs: None,
        code=lambda *args, **kwargs: None,
        # Fixed: slider now accepts positional arguments
        slider=lambda *args, **kwargs: kwargs.get(
            "value", args[3] if len(args) > 3 else 0.01
        ),
        text_input=lambda *args, **kwargs: kwargs.get("value", "0,0,0"),
        text_area=lambda *args, **kwargs: kwargs.get(
            "value", "0,0,0,0,0,0"
        ),  # 6D state
    )
    st.sidebar = sidebar
...
def install_fake_modules(monkeypatch):
    ...
    class DummyController:
        def compute_control(self, state, sv, hist):
            return 0.0, sv, hist
    ...
    class DIP:
        ...
        def step(self, s, u, dt):
            return s  # simple hold
...
def test_app_import_and_main(monkeypatch):
    ...
    monkeypatch.setattr(
        streamlit_app,
        "open",
        lambda *args, **kwargs: io.StringIO("English: {}\n"),
        raising=False,
    )
