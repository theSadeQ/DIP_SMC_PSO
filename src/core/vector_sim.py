...
underlying dynamics step through ``src.core.simulation_runner.step`` and
...
def simulate(
    ...
    *,
    ...
    ...     control_inputs=np.array([0.1, -0.1, 0.0]),
    ...     dt=0.1,
    ...     horizon=3,
    ... )
    >>> x.shape
    ...
    Basic interface validation for batched inputs:

    >>> rng = np.random.default_rng(0)
    >>> x0 = np.zeros((3, 2))  # B=3, D=2
    >>> assert x0.shape == (3, 2)
    >>> controls = np.array([0.1, 0.0])  # H=2 controls
    ...
    for i in range(H):
        ...
        if batch_mode:
            ...
            if u.ndim == 3:
                ...
                # Could be (B,H) or (H,U)
                if u.shape[0] == n_batches and u.shape[1] >= H:
                    u_i = u[:, i]
                else:
                    # (H,U) broadcast across batches
                    u_i = np.broadcast_to(u[i], (n_batches,) + u[i].shape)
            elif u.ndim == 1:
                # Scalar control input per step
                u_i = np.broadcast_to(u[i], (n_batches,))
            else:
                u_i = u[:, i]
        else:
            # Non‑batch: shapes (H,) or (H,U)
            if u.ndim == 1:
                u_i = u[i]
            else:
                u_i = u[i]
...
def simulate_system_batch(
    *,
    controller_factory: Callable[[NDArray[np.float64]], Any],
    particles: ArrayLike,
    sim_time: float,
    dt: float,
