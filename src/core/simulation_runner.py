...
``step(x, u, dt)`` function which calls either ``src.core.dynamics_full.step``
or ``src.core.dynamics_lowrank.step`` depending on the configuration.
...
def get_step_fn():
    ...
    -------
    callable
        Either ``src.core.dynamics_full.step`` or ``src.core.dynamics_lowrank.step``.
    """
