...
ClassicalSMC = _try_import(
    "controllers.classic_smc", "src.controllers.classic_smc", "ClassicalSMC"
)
SuperTwistingSMC = _try_import(
    "controllers.sta_smc", "src.controllers.sta_smc", "SuperTwistingSMC"
)
AdaptiveSMC = _try_import(
    "controllers.adaptive_smc", "src.controllers.adaptive_smc", "AdaptiveSMC"
)
SwingUpSMC = _try_import(
    "controllers.swing_up_smc", "src.controllers.swing_up_smc", "SwingUpSMC"
)
MPCController = _try_import(
    "controllers.mpc_controller", "src.controllers.mpc_controller", "MPCController"
)
MPCWeights = _try_import(
    "controllers.mpc_controller", "src.controllers.mpc_controller", "MPCWeights"
)
...
DoubleInvertedPendulum = _try_import(
    "core.dynamics", "src.core.dynamics", "DoubleInvertedPendulum"
)
FullDIPDynamics = _try_import(
    "core.dynamics_full", "src.core.dynamics_full", "FullDIPDynamics"
)
...
def apply_deprecation_mapping(
    controller_name: str, params: Dict[str, Any], allow_unknown: bool = False
) -> Dict[str, Any]:
    """
    ...
    for original_key, value in params.items():
        ...
        if original_key in deprecation_map:
            ...
            if new_key not in params and new_key not in normalized_params:
                ...
                logger.debug(
                    f"Mapped deprecated param '{original_key}' -> '{new_key}' for {controller_name}"
                )
                norm_key = new_key
            else:
                # New key already set, skip the deprecated one
                logger.debug(
                    f"Skipping deprecated param '{original_key}' as '{new_key}' is already set"
                )
                continue
        # Also check normalized form in deprecation map
            ...
            if new_key not in params and new_key not in normalized_params:
                ...
                logger.debug(
                    f"Mapped deprecated param '{norm_key}' -> '{new_key}' for {controller_name}"
                )
                norm_key = new_key
            else:
                continue
...
def redact_value(value: Any) -> str:
    ...
    if any(
        ...
        for keyword in ["password", "secret", "token", "key", "api"]
    ):
        return "***"
...
def _get_default_gains(
    controller_name: str, config: Any, gains_override: Optional[List[float]]
) -> List[float]:
...
def _validate_shared_params(controller_name: str, dt: float, max_force: float) -> None:
    ...
    if not (isinstance(dt, numbers.Real) and math.isfinite(float(dt)) and dt > 0):
        raise ConfigValueError(
            f"controllers.{controller_name}.dt must be > 0 (got {dt})"
        )
    if not (
        isinstance(max_force, numbers.Real)
        and math.isfinite(float(max_force))
        and max_force > 0
    ):
        raise ConfigValueError(
            f"controllers.{controller_name}.max_force must be finite and > 0 (got {max_force})"
        )
    ...
    *,
    ...
    with CONTROLLER_REGISTRY_LOCK:
        if controller_name in CONTROLLER_REGISTRY:
            ...
            normalized_cfg = apply_deprecation_mapping(
                controller_name, cfg_dict, allow_unknown
            )
            ...
            if gains is not None:
                ...
                try:
                    normalized_cfg["gains"] = _get_default_gains(
                        controller_name, config, None
                    )
                except Exception:
            ...
            try:
                if hasattr(controller_class, "from_config"):
                    ...
                    if allow_unknown:
                        ...
                        filtered = {
                            k: v for k, v in normalized_cfg.items() if k in valid
                        }
                        unknown = {
                            k: v for k, v in normalized_cfg.items() if k not in valid
                        }
                        instance = controller_class(**filtered)
                        if unknown:
                            instance.unknown_params = {
                                **getattr(instance, "unknown_params", {}),
                                **unknown,
                            }
                    else:
    ...
    logger.debug(
        f"Controller '{controller_name}' not in registry, trying legacy construction"
    )
...
def build_all(
    ...
    *,
    ...
    for name, controller_cfg in items:
        try:
            controller = build_controller(
                name, controller_cfg, allow_unknown=allow_unknown, config=config
            )
    ...
    logger.info(
        f"Successfully built {len(built_controllers)} controllers: {list(built_controllers.keys())}"
    )
...
def _legacy_create_controller(
    ...
    if config is None:
        if load_config is None:
            raise RuntimeError(
                "No configuration loader available. Provide 'config' explicitly."
            )
    ...
    if (
        key not in controllers_map
        and key == "classical_smc"
        and "classic_smc" in controllers_map
    ):
        key = "classic_smc"
    ...
    if key not in controllers_map:
        available = (
            "none configured"
            if len(controllers_map) == 0
            else ", ".join(sorted(controllers_map.keys()))
        )
        raise FactoryConfigurationError(
    ...
    if key == "classical_smc":
        ...
        return _build_hybrid_adaptive_sta_smc(
            key,
            ctrl_cfg_dict,
            config,
            gains,
        ...
        raise FactoryConfigurationError(
            f"Controller '{ctrl_name}' is not a recognized type."
        )


# Individual controller builders (legacy)
def _build_classical_smc(
    key,
    ctrl_cfg_dict,
    config,
    gains,
    ...
    if boundary_layer <= 0:
        raise ConfigValueError(
            f"controllers.{key}.boundary_layer must be > 0 (got {boundary_layer})"
        )
...
def _build_sta_smc(
    ...
    instance = SuperTwistingSMC(
        ...
        **({"switch_method": switch_method} if switch_method else {}),
        **(
            {"anti_windup_gain": float(ctrl_cfg_dict["anti_windup_gain"])}
            if "anti_windup_gain" in ctrl_cfg_dict
            else {}
        ),
    )
...
def _build_adaptive_smc(
    key, ctrl_cfg_dict, config, gains, shared_dt, shared_max_force, allow_unknown
):
    ...
    if "boundary_layer" in filtered:
        ...
        if bl <= 0:
            raise ConfigValueError(
                f"controllers.{key}.boundary_layer must be > 0 (got {bl})"
            )
...
def _build_hybrid_adaptive_sta_smc(
    key,
    ctrl_cfg_dict,
    config,
    gains,
...
def _build_mpc_controller(
    ...
    if "horizon" in ctrl_cfg_dict:
        ...
        if not isinstance(hv, int):
            raise ConfigValueError(
                f"controllers.{key}.horizon must be an integer ≥ 1 (got {hv!r})"
            )
        horizon = hv
        if horizon < 1:
            raise ConfigValueError(
                f"controllers.{key}.horizon must be ≥ 1 (got {horizon})"
            )

    max_cart_pos = float(ctrl_cfg_dict.get("max_cart_pos", 1.5))
    if not (math.isfinite(max_cart_pos) and max_cart_pos > 0):
        raise ConfigValueError(
            f"controllers.{key}.max_cart_pos must be > 0 (got {max_cart_pos})"
        )
    ...
    if "max_theta_dev" in ctrl_cfg_dict:
        ...
        if not (math.isfinite(mtd) and mtd > 0):
            raise ConfigValueError(
                f"controllers.{key}.max_theta_dev must be > 0 when provided (got {mtd})"
            )
    ...
    for wkey in ("q_x", "q_theta", "r_u"):
        if wkey in ctrl_cfg_dict:
            ...
            if val < 0:
                raise ConfigValueError(
                    f"controllers.{key}.{wkey} must be ≥ 0 (got {val})"
                )
