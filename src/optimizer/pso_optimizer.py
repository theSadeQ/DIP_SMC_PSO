...
class PSOTuner:
    ...
    def __init__(
        ...
        *,
        ...
        if pso_cfg is not None:
            ...
            # callers to remove outdated keys【738473614585036†L239-L256】.
            deprecated_fields = []
    ...
    def optimise(
        ...
        * **Velocity clamping:** if ``pso_cfg.velocity_clamp`` is a tuple
          ``(δ_min, δ_max)``, the per‑dimension particle velocities are
          constrained to lie within ``δ_min*(bmax - bmin)`` and
          ``δ_max*(bmax - bmin)``【199638979669096†L575-L586】.  Clamping prevents
        ...
        * **Inertia weight scheduling:** if ``pso_cfg.w_schedule`` is provided
          ...
          gradually from global exploration to local exploitation【199638979669096†L294-L301】.
        ...
        # per‑instance Generator avoids global side effects【218697608892619†L682-L687】.
        ...
        # expressed as fractions of the search range and multiplied by ``bmax - bmin``【199638979669096†L575-L586】.
        ...
        # exploration and late exploitation【199638979669096†L294-L301】.  The
        ...
        # inertia weight ``w`` should typically lie in [0.4, 0.9]【199638979669096†L294-L301】.
